...
# Identifying SNP ranges why is this useful?
#snp.list = list()

#for (i in 1 : nrow(input.data)) {
#    if (input.data$type[i] == "SNP" && is.na(input.data$type[i]) == FALSE) {
#        snp.range = c(input.data$mstart[i] : input.data$mstop[i])
#        snp.list <- append(snp.list, snp.range)
#    }
#}

#snp.df <- as.data.frame(table(unlist(snp.list)))
#snp.df$Var1 <- as.numeric(as.character(snp.df$Var1))

# Determining figure format	
plot.title <- paste("Unique Alleles (aa):", INDEX.NO, sep = " ")
png.title <- paste(INDEX.NO, "-macse-aa-uniq.png", sep = "")
...
for (i in 1 : nrow(input.data)) {
    ...
    if (isTRUE(input.data$type[i] == "MS")) {
    ...
    else if (isTRUE(input.data$type[i] == "NS")) {
        rect(input.data$mstart[i]-.05,
            input.data$y1[i],
            input.data$mstop[i]+.05,
            input.data$y2[i],
    ...
    else if (isTRUE(input.data$type[i] == "SYN")) {
        rect(input.data$mstart[i]-.05,
            input.data$y1[i],
            input.data$mstop[i]+.05,
            input.data$y2[i],
            col = "#7C2C8C", border= "#7C2C8C"
        )
    }
